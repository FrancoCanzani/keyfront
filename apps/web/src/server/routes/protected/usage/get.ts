  ...
  async (c) => {
    ...
    const keys = await db
      .select({
        ...
        count: sql`coalesce(sum(${usageRollup.count}) filter (where ${usageRollup.windowStart} >= ${since}), 0)`.mapWith(Number),
        err4: sql`coalesce(sum(${usageRollup.err4Count}) filter (where ${usageRollup.windowStart} >= ${since}), 0)`.mapWith(Number),
        err5: sql`coalesce(sum(${usageRollup.err5Count}) filter (where ${usageRollup.windowStart} >= ${since}), 0)`.mapWith(Number),
        latencyMsSum: sql`coalesce(sum(${usageRollup.latencyMsSum}) filter (where ${usageRollup.windowStart} >= ${since}), 0)`.mapWith(Number),
        monthCount: sql`coalesce(sum(${usageRollup.count}) filter (where ${usageRollup.windowStart} >= ${monthStart}), 0)`.mapWith(Number),
      })
      .from(apiKeys)
      .innerJoin(consumers, eq(apiKeys.consumerId, consumers.id))
      .innerJoin(services, eq(consumers.serviceId, services.id))
